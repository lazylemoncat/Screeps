...
export const claimTask = {
  run: function(room: string) {
    if (Game.rooms[room].controller.owner.username != null &&
        Game.rooms[room].find(FIND_STRUCTURES).filter(structure =>
    ...
    if (Game.flags.claim.room.controller.owner.username == 'LazyKitty') {
